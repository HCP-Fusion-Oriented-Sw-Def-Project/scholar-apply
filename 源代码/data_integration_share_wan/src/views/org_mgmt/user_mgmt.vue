  ...
  <div class="app-container calendar-list-container">
    ...
    <div class="right-container">
      ...
      <div class="filter-container">
        <el-input v-model="listQuery.loginName" class="filter-item" placeholder="账号" @keyup.enter.native="handleFilter" />
        <el-input v-model="listQuery.name" class="filter-item" placeholder="昵称" @keyup.enter.native="handleFilter" />
        <el-input
        ...
        <el-input
          ...
          @keyup.enter.native="handleFilter"
          @change="handleFilter"
          @focus="officeDialogFormVisible=true,officeDialogStatus='officeId'"
        />
      </div>
      <!-- 中部，添加、批量删除等操作-->
      <div class="edit-container">
        <el-button size="small" type="primary" icon="el-icon-plus" @click="handleCreate">添加</el-button>
        <el-button size="small" type="danger" @click="deleteInBatches"><svg-icon icon-class="delete" />批量删除</el-button>
        ...
        <div class="filter-button">
          ...
          <el-button v-waves class="filter-item" type="info" icon="el-icon-refresh" @click="resetListQuery">重置</el-button>
        </div>
      </div>
      <el-table
        :key="tableKey"
      ...
      >
        ...
        <el-table-column min-width="65px" label="用户账号">
          <template slot-scope="scope">
            <span class="link-type" @click="handleUpdate(scope.row)">{{ scope.row.loginName }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column align="center" label="操作" width="230" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            <el-button type="primary" size="mini" style="margin-left:10px" @click="handleUpdate(scope.row)">编辑</el-button>
            ...
            <el-popover placement="top" width="160" trigger="click">
              ...
              <div style="text-align: right; margin: 0">
                <el-button size="mini" type="text" @click="cancelPopover(scope.row, scope.$index)">取消</el-button>
                <el-button type="primary" size="mini" @click="cancelPopover(scope.row, scope.$index),handleDelete(scope.row)">确定</el-button>
              </div>
              <el-button :ref="'btn'+scope.$index" slot="reference" size="mini" type="danger" style="margin-left:10px">删除</el-button>
            </el-popover>
    ...
    <el-dialog v-dialogDrag :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" top="10vh">
      <el-form ref="dataForm" :inline="true" :rules="rules" :model="temp" label-position="right" label-width="100px" style="min-height: 380px">
        <el-col :span="12">
          <!-- <el-form-item label="头像 1233：" prop="photo">
          ...
          <el-form-item label="归属机构 :" prop="officeName">
            <el-input
              ...
              @focus="officeDialogFormVisible=true, officeDialogStatus='tempOfficeId'"
            />
          </el-form-item>
          <el-form-item label="用户角色 :" prop="uacRole">
            <el-select v-model="temp.uacRole" multiple placeholder="请选择" @change="$forceUpdate()">
              <el-option
          ...
          <el-form-item v-if="dialogStatus!=='create'" label="创建时间 :" prop="createDate">
        ...
        <el-col :span="12">
          ...
          <el-form-item label="账号状态 :" prop="status">
            <el-radio-group v-model="temp.status">
              <el-radio label="ENABLE">可用</el-radio>
              <el-radio label="DISABLE">不可用</el-radio>
            </el-radio-group>
          </el-form-item>
          <el-form-item v-if="dialogStatus!=='create'" label="最后登录 ：" prop="lastLoginTime">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button v-if="dialogStatus==='create'" :disabled="submitButtonDisabled" type="primary" @click="createData">确认</el-button>
        <el-button v-else type="primary" @click="updateData">确认</el-button>
      </div>
    </el-dialog>
    <el-dialog title="选择机构" :visible.sync="officeDialogFormVisible" width="30%" height="50%" top="12vh">
      <p>组织机构</p>
      <hr>
      <el-tree
        ref="officeTree"
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="getCurrentNode(), officeDialogFormVisible = false">确认</el-button>
      </div>
    </el-dialog>
...
<script>
import { fetchUserList, updateUser, createUser, deleteLogicalUser, getUserInfoById } from '@/api/user'
...
export default {
  ...
  data() {
    ...
    const validateLoginPwd = (rule, value, callback) => {
      ...
      if (value !== '' && value !== null) {
        if (!/^([A-Za-z]+)(\d+[$@!%*?&<>=+-]+|[$@!%*?&<>=+-]+\d+)[A-Za-z\d$@!%*?&<>=+-]*$/.test(value)) {
    ...
    const validateOfficeName = (rule, value, callback) => {
      value = this.temp.officeName
      if (value == null || value.trim() === '') {
        callback(new Error('请选择归属机构'))
      } else {
        callback()
      }
    }
    return {
      ...
      rules: {
        loginName: [{ required: true, validator: validateLoginName, trigger: ['blur', 'change'] }],
        ...
        loginPwd: [{ required: true, validator: validateLoginPwd, trigger: ['blur', 'change'] }],
        confirmPwd: [{ required: true, validator: validateConfirmPwd, trigger: ['blur', 'change'] }],
        status: [{ required: true, message: '请输入状态', trigger: ['blur', 'change'] }],
        officeName: [{ required: true, validator: validateOfficeName, message: '请选择归属机构', trigger: ['blur'] }],
        uacRole: [{ required: true, validator: validateRoles, trigger: ['blur', 'change'] }]
      },
  ...
  computed: {
    getAllRolesList() {
      ...
      // console.log(this.allRolesList)
      return this.allRolesList.filter((role) => {
        return role.name !== '专家'
      }
      ).map((role) => {
        return Object.assign({ label: role.name + '(' + (role.enname) + ')', value: role.id }, role)
      })
    },
    headers() {
      return {
  ...
  methods: {
    ...
    getOfficeList() {
      fetchOfficeTree().then(response => {
        ...
        // this.officeData = arrayToTree2(response.data.data.list);
      })
    },
      ...
      this.$refs['dataForm'].validate((valid) => {
        ...
        if (valid) {
          ...
          createUser(this.temp).then(response => {
            if (response.data.code === 500) { // 添加失败
              this.$notify({
                title: '成功',
    ...
    handleUpdate(row) {
      getUserInfoById(row.id).then(response => {
        ...
        if (this.temp.officeList !== null && this.temp.officeList !== undefined && this.temp.officeList.length > 0) {
    ...
    handleDelete(row) {
      ...
      deleteLogicalUser(id).then(response => {
        this.$notify({
          title: '成功',
          message: response.msg,
          type: 'success',
    ...
    deleteInBatches() {
      // deleteUserInBatches(this.multipleSelection).then(response =>{
      //   this.$notify({
      //     title: '成功',
      //     message: response.data.msg,
      //     type: 'success',
      ...
      for (var v of this.multipleSelection) {
        if (this.multipleSelection[this.multipleSelection.length - 1] === v) {
          deleteLogicalUser(v).then(response => {
            cout++
            if (cout === this.multipleSelection.length) {
              this.$notify({
                title: '操作成功',
                message: response.msg,
                type: 'success',
